// Under Pressure - shared site behavior (nav, mobile menu, quote funnel)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Mobile accordion toggles (Services / Service Areas)
...
function validateStep1() {
  ...
  if (checks.length === 0) {
    document.getElementById('q-error-1').classList.remove('hidden');
    return;
  }
...
async function submitQuote() {
  ...
  if (!n || !e || !p || !c || !e.includes('@')) {
    document.getElementById('q-error-2').classList.remove('hidden');
    return;
  }
  document.getElementById('q-error-2').classList.add('hidden');

  // Rough rate ranges ($/sq ft) used only to give a ballpark instant estimate.
  let minRate = 0;
  let maxRate = 0;
...
function goToQuoteStep(step) {
  ...
  [1, 2, 3].forEach((i) => {
    ...
    if (i < step) {
      tab.className = 'text-green-500 flex flex-col items-center gap-1';
      circle.className = 'w-8 h-8 rounded-full bg-green-100 text-green-600 flex items-center justify-center font-bold';
      circle.innerHTML = '&#10003;';
    } else if (i === step) {
      tab.className = 'text-brand-orange flex flex-col items-center gap-1';
      circle.className = 'w-8 h-8 rounded-full bg-brand-orange text-white flex items-center justify-center font-bold';
      circle.innerHTML = String(i);
    } else {
      tab.className = 'text-slate-400 flex flex-col items-center gap-1';
      circle.className = 'w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center font-bold';
      circle.innerHTML = String(i);
    }
  });
